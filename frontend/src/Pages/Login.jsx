...
import { ShieldCheck, CheckCircle, Mail, Lock, ArrowRight, Activity, Sun, Moon, Eye, EyeOff } from 'lucide-react';
...
export default function Login({ onLogin, theme, toggleTheme }) {
  ...
  const verifyMfa = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.mfa.challengeAndVerify({ factorId, code: mfaCode });
      if (error) throw error;
  ...
  const subheadings = {
    ...
    register: 'Set up your organisation and get started.',
    reset: isRecovery
      ...
      <div className="login-card-enterprise fade-in-up">
        ...
        <div className="login-brand-side">
          ...
          <div className="brand-content">
            ...
            <p>
              Machine-learning bug triage that learns from your team —
              classify, prioritise, and track defects in one place.
            </p>
        ...
        <div className="login-form-side">
          {viewState === 'success' ? (
            ...
            <div className="fade-in form-content-wrapper" style={{ textAlign: 'center' }}>
              <div style={{
                width: 64, height: 64, background: 'var(--pill-bg)', borderRadius: '50%',
            ...
            <div className="fade-in form-content-wrapper">
              ...
              <form onSubmit={handleAuth} className="modern-form">
                ...
                <button
                  className="sys-btn full login-btn-ent"
                  type="submit"
                  disabled={isLoading}
              ...
              <div className="login-links-ent" style={{ marginTop: 24 }}>
                {mode === 'login' ? (
                  <>
                    <span onClick={() => switchTo('reset')} style={{ cursor: 'pointer' }}>
                      Forgot password?
                    </span>
                    <span style={{ opacity: 0.25 }}>·</span>
                    <span onClick={() => switchTo('register')} style={{ cursor: 'pointer', color: 'var(--accent)', fontWeight: 600 }}>
                      Create account
                    </span>
                  </>
                ) : (
                  <span onClick={() => switchTo('login')} style={{ cursor: 'pointer' }}>
                    ← Back to sign in
                  </span>
                )}
              </div>
